 ...
 */
@Controller
public class PELSOutcomeDissatController {
	private static final Logger log = LoggerFactory.getLogger(PELSOutcomeDissatController.class);
	 ...
	 * @return
	 */
	@RequestMapping(value= {"/Outcome_Dissat_Search.do"}, method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView outcomeDissatSearch (HttpServletRequest request) {
		...
		int TCNT = pelsOutcomeService.getCount("DissatCount", paramMap);
		int TOTALPAGE = 0;
		...
		ArrayList outcomeList = (ArrayList) pelsOutcomeService.getList("DissatList", paramMap);
		...
		mav.setViewName("/pels/outcome/Outcome_Dissat_Search");
		return mav;
	}
	
	@RequestMapping(value="/Outcome_Dissat_Input.do", method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView outcomeDissatInput (HttpServletRequest request) {
		...
		paramMap.put("TST_UNQ_KY_VAL", StringUtil.nvl(TST_UNQ_KY_VAL,""));
		paramMap.put("PPCD", StringUtil.nvl(PPCD,""));
		
		Map<String, String> examDetail = pelsOutcomeService.getDetail("ExamDetail", paramMap);
		if(examDetail != null) {
			mav.addObject("CHCK_STRT_DT", examDetail.get("FM_CHCK_STRT_DT"));
			mav.addObject("CHCK_END_DT", examDetail.get("FM_CHCK_END_DT"));
			mav.addObject("PRCDOC_NO", examDetail.get("PRCDOC_NO"));
			mav.addObject("PRCDOC_NM", examDetail.get("PRCDOC_NM"));
			mav.addObject("TITL_NM", examDetail.get("TITL_NM"));
			mav.addObject("ATCT_NM", examDetail.get("ATCT_NM"));
		}
		...
		mav.setViewName("/pels/outcome/Outcome_Dissat_Input");
		
		return mav;
	}
	
	@RequestMapping(value="/Outcome_Dissat_Detail.do", method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView outcomeDissatDetail (HttpServletRequest request) {
		...
		Map<String, String> DissatDetail = pelsOutcomeService.getDetail("DissatDetail", paramMap);

		mav.addObject("DissatDetail", DissatDetail); // 등록자명
		
		mav.setViewName("/pels/outcome/Outcome_Dissat_Detail");
