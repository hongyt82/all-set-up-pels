...
@Controller
public class PELSWordsController {

	private static final Logger log = LoggerFactory.getLogger(PELSWordsController.class);
	 ...
	 * @return
	 */
	@RequestMapping(value= {"/Words_Search.do", "/Words_Search_M.do"}, method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView WordsSearch (HttpServletRequest request) {
		...
		int TCNT = pelsProcedureService.getCount("WordsCount", paramMap); // 총 조회수
		int TOTALPAGE = 0;
		...
		ArrayList wordsList = (ArrayList) pelsProcedureService.getList("WordsList", paramMap); // 정주기시험 리스트
		if ("/Words_Search_M.do".equals(request.getRequestURI())) {
			HashMap<String, Object> paramMap2 = new HashMap<String, Object>();
			paramMap2.put("wordsList", wordsList);
			...
			if(Detail != null) {
				mav.addObject("PRCDOC_NO", Detail.get("PRCDOC_NO"));
				mav.addObject("PRCDOC_NM", Detail.get("PRCDOC_NM"));
				mav.addObject("ATCT_NM", Detail.get("ATCT_NM"));
			}
			...
			mav.setViewName("/pels/system/Words_Search");
		}
		
		return mav;
	}
	
	@RequestMapping(value="/Words_Input.do", method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView WordsInput (HttpServletRequest request) {
		...
		mav.setViewName("/pels/system/Words_Input");
		return mav;
	}
	
	@RequestMapping(value={"/Words_Insert_Ajax.do"} , method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public Map<String, String> WordsSave (HttpServletRequest request) throws Exception {
		...
		String WRGD_CTT = jsonXssFilter.cleanXSS(StringUtil.nvl(request.getParameter("WRGD_CTT"), "")); 
		
		HashMap<String, Object> paramMap = new HashMap<String, Object>();
		
		paramMap.put("PRCDOC_UNQ_KY_VAL", PRCDOC_UNQ_KY_VAL);
		paramMap.put("WRGD_CTT", WRGD_CTT);
		...
		try {
			pelsProcedureService.insert("InsertWords", paramMap);
		...
		resultMap.put("callMethod", "wordsSave");
	...
	@RequestMapping(value="/Words_Delete_Ajax.do", method = {RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public Map<String, String> WordsDelete (HttpServletRequest request) {
		Map<String, String> resultMap = new HashMap<String, String>();
		String PRCDOC_UNQ_KY_VAL = jsonXssFilter.cleanXSS(StringUtil.nvl(request.getParameter("PRCDOC_UNQ_KY_VAL"), ""));
		String WRGD_CTT = jsonXssFilter.cleanXSS(StringUtil.nvl(request.getParameter("WRGD_CTT"), ""));
		...
		try {
			resultCnt = pelsProcedureService.delete("DeleteWords", map);	
