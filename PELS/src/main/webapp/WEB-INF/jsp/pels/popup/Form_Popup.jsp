...
<div class="Header">  
	...
	<c:if test="${'M' eq PRCDOC_CFY}">		
		<span class="Text">점검지 선택</span>
	</c:if>	
	<c:if test="${'P' eq PRCDOC_CFY}">		
		<span class="Text">정주기시험 선택</span>
	</c:if>	
	</div>      
			...
			<div class="Grid">
				<table cellspacing="0" cellpadding="0" border="0" class="Outline">
					...
					<tr class="Header">
						...
						<th>절차서번호</th>
						<c:if test="${'M' eq PRCDOC_CFY}">		
						<th>절차서명<font color="blue">[점검지명]</font></th>
						</c:if>	
						<c:if test="${'P' eq PRCDOC_CFY}">		
						<th>절차서명</th>
						</c:if>	
					...
					<c:forEach var="form" items="${formList}" begin="0" end="${formList.size()}" step="1">
						<tr class="Item">
							...
							<td align="left">${form.PRCDOC_NM} <c:if test="${not empty form.ATCT_NM}"><font color="blue">[${form.ATCT_NM}]</font></c:if></td>
