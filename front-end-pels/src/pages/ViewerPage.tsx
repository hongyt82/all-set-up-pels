...
import { exportFilledPdf } from '../lib/pdf/exportFilledPdf';
import {
  ViewerWorkspace,
...
import { decodeRuleExpressions } from '../utils/ruleHtmlDecode';
...
export function ViewerPage() {
  ...
  const applyRuleJsonObject = (obj: any) => {
    ...
    const decodedObj = decodeRuleExpressions(obj);

    if (!decodedObj.docId) decodedObj.docId = 'UNKNOWN';

    devLog('[Rule] apply', {
      pages: decodedObj.pages.length,
      treelist: decodedObj.treelist?.length,
    });

    setConstraintDoc(decodedObj);
  };
